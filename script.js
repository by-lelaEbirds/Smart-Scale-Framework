document.addEventListener('DOMContentLoaded', () => {
    ...
    function executarOferta(option) {
        ...
        if (option === 'A') {
            ...
            const newSells = currentState.sells + 1;
            // "Mágica de apresentação" para o Ticket Médio ter um aumento visível
            const newTicket = currentState.ticket + (Math.random() * 1.5 + 0.5);
            
            // Anima para os novos valores
            animateKpi(kpis.crossSells, newSells);
            animateKpi(kpis.ticket, newTicket, true);

            // Atualiza o estado interno
            currentState.sells = newSells;
            currentState.ticket = newTicket;

        } else { // Opção B
            // RECUSA GARANTIDA
            addToLog("RESULTADO: Cliente recusou a Oferta B. Indicadores inalterados.");
            // Nenhum indicador é atualizado
        }
